...
export const NotificationProvider = ({ children }) => {
  ...
  const addNotification = useCallback((notification) => {
    ...
    const clientId =
      notification.clientId || `client-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

    setNotifications((prev) => {
      const isDuplicate = prev.some(
        (n) => (n._id && n._id === notification._id) || n.clientId === clientId
      );
  ...
  const markAsRead = useCallback(async (notificationId) => {
    try {
      ...
      if (!isValidObjectId) {
        ...
        setNotifications((prev) =>
          prev.map((n) =>
            n.id === notificationId || n._id === notificationId || n.clientId === notificationId
              ? { ...n, read: true }
              : n
          )
        );
  ...
  const removeNotification = useCallback(async (notificationId) => {
    try {
      ...
      if (!isValidObjectId) {
        console.log(
          'Skipping API call for client-generated notification deletion:',
          notificationId
        );
        setNotifications((prev) => {
  ...
  const handleInvoiceCreated = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Invoice #${data.invoiceNumber || ''} created for your order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
  ...
  const handleInvoicePaymentUpdated = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Payment of ₦${data.amountPaid?.toLocaleString() || ''} received for your order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.invoiceId ? `/dashboards/customer/invoices/${data.invoiceId}` : '/invoices',
  ...
  const handleOrderStatusUpdate = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message || `Your order #${data.orderNumber} is now ${data.status || 'updated'}`,
        data,
        timestamp: new Date(),
        read: false,
  ...
  const handleDesignUploaded = useCallback(
    (data) => {
  ...
  const handleDesignApproved = useCallback(
    (data) => {
  ...
  const handleAdminBriefResponse = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Admin responded to your customization request for order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
  ...
  const handleBriefDeleted = useCallback(
    (data) => {
  ...
  const handleFeedbackStatusUpdated = useCallback(
    (data) => {
  ...
  const handlePaymentVerified = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Your payment of ₦${data.amount?.toLocaleString() || ''} has been verified`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId ? `/orders/${data.orderId}` : '/orders',
  ...
  const handlePaymentFailed = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Your payment of ₦${data.amount?.toLocaleString() || ''} failed. ${data.reason || ''}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId ? `/orders/${data.orderId}` : '/orders',
  ...
  const handleReceiptUploaded = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Your receipt for order #${data.orderNumber} has been uploaded and is pending verification`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId ? `/orders/${data.orderId}` : '/orders',
  ...
  const handleBankTransferApproved = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Your bank transfer of ₦${data.amount?.toLocaleString() || ''} has been approved`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId ? `/orders/${data.orderId}` : '/orders',
  ...
  const handleBankTransferRejected = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Your bank transfer of ₦${data.amount?.toLocaleString() || ''} was rejected. ${data.reason || ''}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId ? `/orders/${data.orderId}` : '/orders',
  ...
  const handleShippingCreated = useCallback(
    (data) => {
  ...
  const handlePickupReady = useCallback(
    (data) => {
  ...
  const handleTrackingUpdated = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Tracking #${data.trackingNumber} added for your order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
  ...
  const handleShippingStatusUpdated = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Shipping status for your order #${data.orderNumber} is now ${data.status}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId ? `/orders/${data.orderId}` : '/orders',
  ...
  const handleShippingPaid = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message || `Your shipping payment for order #${data.orderNumber} has been received`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId ? `/orders/${data.orderId}` : '/orders',
  ...
  const handleAdminNewOrder = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        read: false,
        link: data.orderId
          ? `/dashboards/admin/orders/${data.orderId}`
          : '/dashboards/admin/orders',
      };
      addNotification(notification);
  ...
  const handleAdminInvoiceCreated = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Invoice #${data.invoiceNumber || ''} created for order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.invoiceId
          ? `/dashboards/admin/invoices/${data.invoiceId}`
          : '/dashboards/admin/invoices',
      };
      addNotification(notification);
      showToast(notification.message, 'success', 5000);
    },
  ...
  const handleAdminInvoiceUpdated = useCallback(
    (data) => {
  ...
  const handleAdminInvoiceSent = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Invoice #${data.invoiceNumber || ''} sent to customer for order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.invoiceId
  ...
  const handleAdminInvoiceDeleted = useCallback(
    (data) => {
  ...
  const handleAdminPaymentReceived = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Payment of ₦${data.amount?.toLocaleString() || ''} received from ${data.customerName || 'customer'}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId
  ...
  const handleAdminPendingBankTransfer = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Bank transfer of ₦${data.amount?.toLocaleString() || ''} from ${data.customerName || 'customer'} requires verification`,
        data,
        timestamp: new Date(),
        read: false,
  ...
  const handleAdminDesignUploaded = useCallback(
    (data) => {
  ...
  const handleAdminDesignApproved = useCallback(
    (data) => {
  ...
  const handleAdminBriefResponded = useCallback(
    (data) => {
  ...
  const handleAdminFeedbackResolved = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        timestamp: new Date(),
        read: false,
        link: data.feedbackId
          ? `/dashboards/admin/feedback/${data.feedbackId}`
          : '/dashboards/admin/feedback',
      };
      addNotification(notification);
      showToast(notification.message, 'success', 5000);
    },
  ...
  const handleAdminShippingCreated = useCallback(
    (data) => {
  ...
  const handleAdminTrackingUpdated = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        title: 'Tracking Updated',
        message:
          data.message || `Tracking #${data.trackingNumber} added for order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId
  ...
  const handleAdminShippingStatusUpdated = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Shipping status for order #${data.orderNumber} changed to ${data.status}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId
  ...
  const handleAdminShippingPaid = useCallback(
    (data) => {
      ...
      const notification = {
        ...
        message:
          data.message ||
          `Shipping payment of ₦${data.cost?.toLocaleString() || ''} received for order #${data.orderNumber}`,
        data,
        timestamp: new Date(),
        read: false,
        link: data.orderId
  ...
  useEffect(() => {
    ...
    handleInvoiceSent,
    handleInvoicePaymentUpdated,
    handleInvoiceDeleted,
    handleOrderStatusUpdate,
    handleDesignUploaded,
    handleDesignUpdated,
    handleDesignApproved,
    handleAdminBriefResponse,
    handleBriefDeleted,
    ...
    handleBankTransferApproved,
    handleBankTransferRejected,
    ...
    handleShippingPaid,
    handleAdminNewOrder,
    handleAdminInvoiceCreated,
    handleAdminInvoiceUpdated,
    handleAdminInvoiceSent,
    handleAdminInvoiceDeleted,
    handleAdminPaymentReceived,
    handleAdminPendingBankTransfer,
    handleAdminDesignUploaded,
    handleAdminDesignApproved,
    handleAdminBriefResponded,
    handleAdminFeedbackResolved,
    handleAdminShippingCreated,
    handleAdminTrackingUpdated,
    handleAdminShippingStatusUpdated,
    handleAdminShippingPaid,
  ]);
