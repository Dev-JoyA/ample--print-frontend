        ...
        <DashboardLayout userRole={user?.role}>
          <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div className="rounded-xl border border-red-700 bg-red-900/30 p-8 text-center">
              ...
              <p className="mb-6 text-gray-400">{error || "The order you're looking for doesn't exist."}</p>
        ...
        <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 pb-12">
          <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
            ...
            <div className="mb-6 rounded-2xl border border-gray-800 bg-gradient-to-br from-slate-900 to-slate-950 p-6 sm:p-8">
              <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-center gap-4">
                  <div className={`flex h-14 w-14 items-center justify-center rounded-2xl ${getStatusColor(order.status)}/20`}>
            ...
            <div className="mb-6 border-b border-gray-800">
              <nav className="flex gap-4 overflow-x-auto pb-1 sm:gap-6 scrollbar-hide">
                <button
            ...
            <div className="space-y-6">
              {activeTab === 'items' && (
                <div className="space-y-4">
                  {order.items?.map((item, index) => {
                    ...
                    return (
                      <div key={index} className="rounded-xl border border-gray-800 bg-slate-900/50 p-4 transition-all hover:border-gray-700 sm:p-6">
                        <div className="flex flex-col gap-4 sm:flex-row">
                          ...
                          <div className="flex-1">
                            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                              <h3 className="text-base font-semibold text-white sm:text-lg">{item.productName}</h3>
                              <p className="text-base font-bold text-red-400 sm:text-lg">{formatCurrency(item.price * item.quantity)}</p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4 sm:gap-4">
                              ...
                              {item.productSnapshot?.material && (
                                <div>
                                  ...
                                  <p className="font-medium text-white">{item.productSnapshot.material}</p>
                                </div>
                              )}
                            </div>
              ...
              {activeTab === 'briefs' && (
                <div className="space-y-6">
                  {order.items?.map((item) => {
                    ...
                    return (
                      <div key={productId} className="overflow-hidden rounded-xl border border-gray-800 bg-slate-900/50 transition-all hover:border-gray-700">
                        ...
                        <div className="p-4 sm:p-6">
                          {brief ? (
                            <div className="space-y-4">
                              {brief.description && (
                                <div>
                                  <p className="mb-1 text-xs text-gray-400 sm:text-sm">Description:</p>
                                  <p className="rounded-lg bg-slate-800/50 p-3 text-sm text-white sm:text-base">{brief.description}</p>
                                </div>
                              )}
                                ...
                                {brief.voiceNote && (
                                  <div className="text-center">
                                    <div className="mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-lg bg-green-900/30 sm:h-12 sm:w-12">
                                      <svg className="h-5 w-5 text-green-400 sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {brief.video && (
                                  <div className="text-center">
                                    <div className="mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-lg bg-red-900/30 sm:h-12 sm:w-12">
                                      <svg className="h-5 w-5 text-red-400 sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {brief.logo && (
                                  <div className="text-center">
                                    <div className="mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-lg bg-purple-900/30 sm:h-12 sm:w-12">
                                      <svg className="h-5 w-5 text-purple-400 sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                      </svg>
                                    </div>
                            ...
                            <div className="py-6 text-center sm:py-8">
                              <p className="text-sm text-gray-400 sm:text-base">No customization brief submitted for this product yet.</p>
                              <Link href={`/orders/${orderId}/products/${productId}/briefs`}>
              ...
              {activeTab === 'payment' && (
                <div className="rounded-xl border border-gray-800 bg-slate-900/50 p-4 sm:p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap justify-between items-center border-b border-gray-800 py-3">
                    ...
                    <div className="flex flex-wrap justify-between items-center border-b border-gray-800 py-3">
                      ...
                      <span className="font-medium text-green-400">{formatCurrency(order.amountPaid)}</span>
                    </div>
                    <div className="flex flex-wrap justify-between items-center py-3">
                      <span className="text-sm text-gray-400 sm:text-base">Remaining Balance</span>
                      <span className="font-medium text-yellow-400">{formatCurrency(order.remainingBalance)}</span>
                    </div>
                    ...
                    <div className="mt-6 rounded-lg bg-slate-800/30 p-4">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className="font-medium text-white">Payment Status: {order.paymentStatus}</span>
                      </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-3 sm:gap-4">
                      <div className="relative">
                        ...
                        {order.updatedAt && <div className="absolute left-3 top-10 h-16 w-0.5 bg-gray-700 sm:left-4"></div>}
                      </div>
                      <div className="flex-1 pb-8">
                        <p className="font-medium text-white">Order Placed</p>
                        <p className="text-xs text-gray-400 sm:text-sm">{formatDate(order.createdAt)}</p>
                        <p className="mt-1 text-xs text-gray-500">Your order has been received and is being processed.</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 sm:gap-4">
                      <div className={`flex h-8 w-8 items-center justify-center rounded-full sm:h-10 sm:w-10 ${getStatusColor(order.status).replace('bg-', 'bg-')}/20`}>
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-400 sm:text-sm">Updated {formatDate(order.updatedAt)}</p>
                      </div>
                    </div>
                  </div>
                </div>
