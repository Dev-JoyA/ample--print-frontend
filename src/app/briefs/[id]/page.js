...
export default function BriefResponseDetailPage({ params }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (!hasLeftPageRef.current && brief && !hasAutoMarked) {
        ...
        if (shouldAutoMark) {
          customerBriefService.markAsViewed(brief._id)
  ...
  const fetchBriefAndConversation = async () => {
    try {
      ...
      if (!briefData) { setNotFound(true); return; }
  ...
  const hasCustomerRespondedAfter = latestCustomer && latestAdmin &&
  ...
  const canRespond = hasAdminResponse && !hasCustomerRespondedAfter && isEditable && latestAdmin?.status === 'responded';
  const canMarkComplete = hasAdminResponse && !isAdminComplete && !hasCustomerRespondedAfter && isEditable;
  ...
  const removeFile = (type, index = null) => {
    if (type === 'images') setResponseFiles(prev => ({ ...prev, images: prev.images.filter((_, i) => i !== index) }));
  ...
  const getRoleBadge = (role) => {
    ...
    if (r === 'customer') return <span className="rounded-full bg-blue-600/20 px-2 py-0.5 text-xs text-blue-400">You</span>;
    if (r === 'admin') return <span className="rounded-full bg-green-600/20 px-2 py-0.5 text-xs text-green-400">Admin</span>;
    ...
    return (
      <DashboardLayout userRole="customer">
        <div className="mx-auto max-w-2xl px-4 py-16 text-center">
          ...
          <p className="mb-6 text-gray-400">{error || "This brief doesn't exist or has been removed."}</p>
          <Link href="/dashboards"><Button variant="primary">Back to Dashboard</Button></Link>
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout userRole="customer">
      ...
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="mb-6 rounded-xl border border-gray-800 bg-slate-900/50 overflow-hidden">
          ...
          <div className="max-h-[480px] overflow-y-auto space-y-4 p-5">
            {loadingConversation ? (
              ...
              messages.map((msg, idx) => (
                <div key={msg._id || idx} className={`flex ${isAdminMsg(msg.role) ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[85%] rounded-xl p-4 ${isAdminMsg(msg.role)
                    ...
                    <div className="flex flex-wrap gap-2">
                      {msg.image && (
                        <button onClick={() => setPreviewImage(getImageUrl(msg.image))}
                          className="inline-flex items-center gap-1 rounded-lg bg-slate-700/50 px-3 py-1.5 text-xs text-blue-400 hover:bg-slate-700 transition">
                          🖼️ View Image
                        </button>
                      )}
                      {msg.voiceNote && (
                      ...
                      {msg.video && (
                        <button onClick={() => setPreviewVideo(getImageUrl(msg.video))}
                          className="inline-flex items-center gap-1 rounded-lg bg-slate-700/50 px-3 py-1.5 text-xs text-red-400 hover:bg-slate-700 transition">
                          🎥 Play Video
                        </button>
                      )}
                      {msg.logo && (
                        <button onClick={() => setPreviewImage(getImageUrl(msg.logo))}
                          className="inline-flex items-center gap-1 rounded-lg bg-slate-700/50 px-3 py-1.5 text-xs text-purple-400 hover:bg-slate-700 transition">
                          🎨 View Logo
                        </button>
                      )}
                    </div>
            ...
            {!showResponseForm ? (
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  {canMarkComplete && (
                    <Button variant="primary" size="md" className="flex-1"
                      onClick={markAsComplete} disabled={markingComplete}>
              ...
              <div className="p-5">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <label htmlFor="img-upload" className="flex cursor-pointer items-center gap-1 rounded-lg bg-slate-800 px-3 py-2 text-xs text-blue-400 hover:bg-slate-700 transition">
                    🖼️ Images
                    <input id="img-upload" type="file" accept="image/*" multiple className="hidden" ref={fileInputRef} onChange={e => handleFileChange(e, 'images')} />
                  </label>
                  <label htmlFor="vid-upload" className="flex cursor-pointer items-center gap-1 rounded-lg bg-slate-800 px-3 py-2 text-xs text-red-400 hover:bg-slate-700 transition">
                    🎥 Video
                    <input id="vid-upload" type="file" accept="video/*" className="hidden" ref={videoInputRef} onChange={e => handleFileChange(e, 'video')} />
                  </label>
                ...
                {(responseFiles.images.length > 0 || responseFiles.video) && (
                  <div className="mt-3 flex flex-wrap gap-2 rounded-lg bg-slate-800/50 p-3">
                    {responseFiles.images.map((f, i) => (
                      <div key={i} className="flex items-center gap-1 rounded-lg bg-slate-700 px-2 py-1">
        ...
        {isAdminComplete && !hasCustomerRespondedAfter && !showResponseForm && (
          <div className="rounded-lg border border-green-800 bg-green-900/20 p-4 text-center">
            <p className="text-sm text-green-400">
              ✓ This response has been marked as complete. The order will proceed when all items are ready.
            </p>
          </div>
        )}
      </div>
