...
export default function ProductBriefThread({
  orderId,
  productId,
  productName,
  orderNumber,
  ...
  const canMarkComplete =
    hasAdminResponse && !isAdminComplete && !customerRespondedAfter && isEditable;
  ...
  const removeFile = (type, idx = null) => {
    if (type === 'images')
      setResponseFiles((prev) => ({ ...prev, images: prev.images.filter((_, i) => i !== idx) }));
    else if (type === 'video') {
    ...
    <div className="space-y-6">
        ...
        <div className="max-h-[480px] space-y-4 overflow-y-auto p-5">
          {messages.length === 0 ? (
            ...
            messages.map((msg, idx) => (
              <div
                key={msg._id || idx}
                className={`flex ${isAdminRole(msg.role) ? 'justify-start' : 'justify-end'}`}
              >
                <div
                ...
                >
                  ...
                  {msg.designId && (
                    <Link href={`/designs/${msg.designId}`}>
                      <span className="inline-flex items-center gap-1 rounded-full bg-purple-900/30 px-3 py-1 text-xs text-purple-400 transition hover:bg-purple-900/50">
                        🎨 View Design
                      </span>
                    </Link>
                  )}
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {msg.image && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-lg bg-slate-700/50 px-3 py-1 text-xs text-blue-400 transition hover:bg-slate-700"
                      >
                        📷 Image
                      </button>
                    )}
                    ...
                    {msg.video && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-lg bg-slate-700/50 px-3 py-1 text-xs text-red-400 transition hover:bg-slate-700"
                      >
                        🎥 Video
                      </button>
                    )}
                    {msg.logo && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-lg bg-slate-700/50 px-3 py-1 text-xs text-purple-400 transition hover:bg-slate-700"
                      >
                        🎨 Logo
                      </button>
                    )}
          ...
          {!showResponseForm ? (
            ...
            <div className="p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <label className="flex cursor-pointer items-center gap-1 rounded-lg bg-slate-800 px-3 py-2 text-xs text-blue-400 transition hover:bg-slate-700">
                  🖼️ Images
                  <input
                    type="file"
                    accept="image/*"
                ...
                <label className="flex cursor-pointer items-center gap-1 rounded-lg bg-slate-800 px-3 py-2 text-xs text-red-400 transition hover:bg-slate-700">
                  🎥 Video
                  <input
                    type="file"
                    accept="video/*"
                ...
                {audioBlob && (
                  <div className="flex items-center gap-2 rounded-lg bg-slate-800 px-3 py-2">
                    ...
                    <button onClick={discardRecording} className="text-gray-400 hover:text-red-400">
                      ✕
                    </button>
                  </div>
                )}
      ...
      {isAdminComplete && !customerRespondedAfter && !showResponseForm && (
        <div className="rounded-lg border border-green-800 bg-green-900/20 p-4 text-center">
          <p className="text-sm text-green-400">✓ This response has been marked as complete.</p>
        </div>
      )}
