...
export default function OrderHistoryDetailPage() {
  ...
  const canDeleteOrder = order?.status
    ? ['Pending', 'OrderReceived'].includes(order.status)
    : false;
          ...
          {message && (
            <div className="mb-6 rounded-lg border border-green-800 bg-green-900/20 p-4">
              <p className="text-green-400">
                ✓ Shipping information saved successfully! An admin will review your shipping
                selection.
              </p>
            </div>
          )}
          ...
          <div className="mb-6 rounded-xl border border-gray-800 bg-slate-900/50 p-6 backdrop-blur-sm">
            ...
            <div className="space-y-4">
              {order.items?.map((item, index) => (
                <div
                  key={index}
                  className="flex flex-col justify-between gap-4 rounded-lg bg-slate-800/30 p-4 sm:flex-row sm:items-center"
                >
                  <div>
                ...
                {shipping.shippingMethod === 'delivery' && shipping.deliveryType === 'prepaid' && (
                  <div className="mt-4 rounded-lg border border-yellow-800 bg-yellow-900/20 p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm text-yellow-400">
                        <span className="font-medium">Shipping invoice pending:</span> An admin will
                        generate a shipping invoice soon. You'll be notified when it's ready for
                        payment.
                      </p>
                    </div>
                  </div>
                ...
                {orderInvoice.remainingAmount > 0 && (
                  <div className="flex flex-col justify-between gap-2 sm:flex-row">
                    ...
                    <p className="font-bold text-yellow-400">
                      {formatCurrency(orderInvoice.remainingAmount)}
                    </p>
                  </div>
                )}
