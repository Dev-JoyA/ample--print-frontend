...
import Textarea from '@/components/ui/Textarea';
import SEOHead from '@/components/common/SEOHead';
import DesignSelectionModal from '@/components/ui/DesignSelectionModal';
...
export default function ProductDetailPage() {
  ...
  const [isDesignChoiceLocked, setIsDesignChoiceLocked] = useState(false);
  ...
  const fetchUserActiveOrders = async () => {
    try {
      ...
      const activeStatuses = ['Pending', 'OrderReceived', 'FilesUploaded'];
      const filteredOrders = orders.filter((order) => activeStatuses.includes(order.status));

      setUserActiveOrders(filteredOrders);
    } catch (err) {
      console.error('Failed to fetch active orders:', err);
    } finally {
      setLoadingOrders(false);
  ...
  const handleDesignSelection = (hasDesign) => {
    setHasOwnDesign(hasDesign);
    setIsDesignChoiceLocked(true);
  ...
  const handlePriceAlertConfirm = () => {
    ...
    // Validate based on design choice
    ...
    if (hasOwnDesign === true) {
      ...
      if (!hasRequiredInfo) {
        setError('Please provide design instructions or reference images');
        return;
      }
    } else {
    ...
    // Also need basic specs
    const hasBasicSpecs = size.trim() !== '' || color.trim() !== '' || quantity > 0;

    if (!hasBasicSpecs && !hasRequiredInfo) {
      setError('Please provide at least one customization detail');
      return;
    }
  ...
  const handleSubmit = async (selectedOrderId) => {
    try {
      ...
      // Add hasOwnDesign flag
      formData.append('hasOwnDesign', hasOwnDesign === true ? 'true' : 'false');

      if (hasOwnDesign === true) {
        // Customer has their own design - upload design files only
        imagery.forEach((file) => formData.append('image', file));
        const description = `Customer has their own design. 
Files uploaded: ${imagery.length} file(s)
...
DESIGN REQUIREMENTS:
-------------------
• Design Assistance Required: Yes
• Size/Dimensions: ${size || 'Not specified (will use standard)'}
• Color: ${color || 'Not specified (to be discussed)'}
• Quantity: ${quantity} units
• Material: ${product.material || 'Standard'}
...
${new Date().toLocaleString()}
  ...
  return (
    <>
            ...
            <div className="mt-4 rounded-lg border border-yellow-800 bg-yellow-900/20 p-3">
              <div className="flex items-start gap-2">
                ...
                <p className="text-xs text-yellow-400">
                  <span className="font-medium">Design Fee Note:</span> If you require design
                  assistance, an additional design fee will be added to your invoice. Providing your
                  own ready-to-print design files helps avoid this extra cost.
                </p>
              </div>
            </div>
      ...
      {/* Design Selection Modal - Using the imported component */}
      <DesignSelectionModal
        isOpen={showDesignModal}
        onClose={() => setShowDesignModal(false)}
        onConfirm={handleDesignSelection}
                ...
                >
                  ...
                  <span className="font-medium">
                    {hasOwnDesign === true
                      ? 'Upload Design Files'
                      : hasOwnDesign === false
                        ? 'Provide Design Instructions'
                        : 'Upload Logo or Design Files'}
                  </span>
                </button>
              </div>
              ...
              {showUploadSection && (
                <div className="space-y-6 border-t border-gray-800 pt-4">
                  {hasOwnDesign === true ? (
                    ...
                    <>
                      <div>
                        ...
                        <p className="mb-3 text-xs text-gray-400">
                          Upload your ready-to-print design files (AI, EPS, PDF, PSD, PNG, JPG)
                        </p>

                        <div className="rounded-lg border-2 border-dashed border-gray-700 p-6 text-center transition-colors hover:border-primary">
                          <input
                            type="file"
                          ...
                          <label htmlFor="design-upload" className="cursor-pointer">
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <span className="text-xs text-gray-500">
                                AI, EPS, PDF, PSD, PNG, JPG (max 20MB each)
                              </span>
                            </div>
                          </label>
                        </div>
                        ...
                        {imageryPreviews.length > 0 && (
                          <div className="mt-3">
                            <p className="mb-2 text-sm text-gray-400">Uploaded files:</p>
                    ...
                    // Customer needs design help - Show full customization
                    <>
                      <div>
                        <h3 className="mb-3 font-semibold text-white">DESIGN INSTRUCTIONS</h3>
                        <Textarea
