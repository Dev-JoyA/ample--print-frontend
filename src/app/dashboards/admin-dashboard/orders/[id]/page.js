      ...
      <DashboardLayout userRole="admin">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <div className="mb-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-2">Order Details</h1>
                  <p className="text-gray-400 text-sm sm:text-base lg:text-lg">Order #{order.orderNumber}</p>
                </div>
              </div>
            ...
            <nav className="flex gap-4 sm:gap-6 min-w-max">
              ...
              >
                ...
                {itemsWithBriefs.some(i => i.hasBrief) && (
                  <span className="ml-1 sm:ml-2 bg-primary/20 text-primary px-1.5 sm:px-2 py-0.5 rounded-full text-xs">
              ...
              <div className="bg-slate-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-5 sm:p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                  <div>
                    ...
                    <p className="text-white text-base sm:text-lg">{getCustomerName()}</p>
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm text-gray-400 mb-1">Email</p>
                    <p className="text-white text-base sm:text-lg">{getCustomerEmail()}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-5 sm:p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6">
                  <div>
                    ...
                    <p className="text-2xl sm:text-3xl font-bold text-primary">{formatCurrency(order.totalAmount)}</p>
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm text-gray-400 mb-1">Amount Paid</p>
                    <p className="text-xl sm:text-2xl font-bold text-green-400">{formatCurrency(order.amountPaid || 0)}</p>
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm text-gray-400 mb-1">Remaining Balance</p>
                    <p className={`text-xl sm:text-2xl font-bold ${order.remainingBalance > 0 ? 'text-yellow-400' : 'text-green-400'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 mt-4">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm text-gray-400 mb-1">Order Date</p>
                    <p className="text-white text-sm sm:text-base">{formatDate(order.createdAt)}</p>
                  </div>
                </div>
              </div>
                    ...
                    <div key={index} className="bg-slate-800/30 rounded-lg p-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex-1">
                          <h3 className="text-white font-medium text-base sm:text-lg">{item.productName}</h3>
          ...
          {activeTab === 'briefs' && (
            <div className="space-y-5 sm:space-y-6">
              {itemsWithBriefs.filter(i => i.hasBrief).length === 0 ? (
                <div className="bg-slate-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-8 sm:p-12 text-center">
                  <div className="text-5xl sm:text-6xl mb-4 opacity-50">📋</div>
                  <h3 className="text-lg sm:text-xl font-semibold text-white mb-2">No Customization Briefs</h3>
                  <p className="text-sm sm:text-base text-gray-400">This order has no customization briefs yet</p>
                </div>
              ) : (
                itemsWithBriefs.filter(i => i.hasBrief).map((item, index) => (
                  <div key={index} className="bg-slate-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-5 sm:p-6">
                    <h3 className="text-base sm:text-lg font-semibold text-white mb-4">{item.productName}</h3>
                    ...
                    {item.briefConversation?.customer && (
                      <div className="mb-6">
                        ...
                        <div className="bg-slate-800/30 rounded-lg p-4">
                          <p className="text-white whitespace-pre-wrap text-sm mb-3">
          ...
          {activeTab === 'shipping' && shipping && (
            <div className="bg-slate-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-5 sm:p-6">
              <h2 className="text-base sm:text-lg font-semibold text-white mb-4">Shipping Information</h2>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                <div>
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Shipping Method</p>
                  <span className={`inline-block px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${
                ...
                <div>
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Shipping Status</p>
                  <span className={`inline-block px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${
              ...
              {shipping.shippingMethod === 'delivery' && (
                <>
                  <div className="mt-5 sm:mt-6">
                    <p className="text-xs sm:text-sm text-gray-400 mb-1">Recipient</p>
                    <p className="text-white text-sm sm:text-base">{shipping.recipientName}</p>
                    <p className="text-xs sm:text-sm text-gray-400">{shipping.recipientPhone}</p>
                  </div>

                  {shipping.address && (
                    <div className="mt-4">
                      <p className="text-xs sm:text-sm text-gray-400 mb-1">Delivery Address</p>
              ...
              {shipping.shippingMethod === 'pickup' && (
                <div className="mt-5 sm:mt-6">
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Pickup Location</p>
                  <p className="text-white text-sm">5, Boyle Street, Somolu, Lagos</p>
                </div>
              )}

              {shipping.trackingNumber && (
                <div className="mt-4">
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Tracking Number</p>
                  <p className="text-white text-sm font-mono">{shipping.trackingNumber}</p>
                  {shipping.carrier && (
                    <p className="text-xs text-gray-400 mt-1">Carrier: {shipping.carrier}</p>
                  )}
                </div>
              )}
              ...
              {shipping.metadata?.pickupNotes && (
                <div className="mt-4">
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Notes</p>
                  <p className="text-white text-sm bg-slate-800/30 rounded-lg p-3">{shipping.metadata.pickupNotes}</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'invoice' && invoice && (
            <div className="bg-slate-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-5 sm:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                <div>
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Invoice Number</p>
                  <p className="text-white text-sm sm:text-base">{invoice.invoiceNumber}</p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Invoice Type</p>
                  <p className="text-white text-sm sm:text-base capitalize">{invoice.invoiceType}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Total Amount</p>
                  <p className="text-xl sm:text-2xl font-bold text-primary">{formatCurrency(invoice.totalAmount)}</p>
                </div>
              </div>

              {invoice.items && (
                <div className="mt-5 sm:mt-6">
                  <p className="text-xs sm:text-sm text-gray-400 mb-3">Items</p>
                  <div className="space-y-2">
                    {invoice.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-slate-800/30 rounded-lg">
                        ...
                        <span className="text-primary font-medium text-sm">{formatCurrency(item.total)}</span>
                      </div>
                    ))}
                  </div>
                </div>
