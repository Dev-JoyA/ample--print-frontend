            ...
            <div className="rounded-lg border border-gray-800 bg-slate-900 p-6">
              ...
              <div className="overflow-x-auto pb-4" style={{ maxWidth: '100%' }}>
                <div
                  ...
                  style={{ minWidth: financialData.monthlyData.length * 80 }}
                >
                  {financialData.monthlyData.map((item, index) => {
                    const maxRevenue = Math.max(
                      ...financialData.monthlyData.map((d) => d.revenue),
                      1
                    );
                    const maxPartPayments = Math.max(
                      ...financialData.monthlyData.map((d) => d.partPaymentsDue),
                      1
                    );
                      ...
                      >
                        <div className="relative h-40 w-full overflow-hidden rounded-lg bg-slate-800">
                          {item.partPaymentsDue > 0 && (
                            ...
                            >
                              <div className="absolute left-1/2 top-0 z-20 -translate-x-1/2 -translate-y-full transform whitespace-nowrap rounded bg-slate-800 px-2 py-1 text-xs text-white opacity-0 transition hover:opacity-100">
                    ...
                    financialData.invoices.slice(0, 10).map((invoice, index) => {
                      ...
                      return (
                        <tr key={index} className="border-t border-gray-800 hover:bg-slate-800/50">
                          ...
                          <td className="p-3">
                            <span
                              className={`rounded-full px-2 py-1 text-xs bg-${getStatusColor(invoice.status)}-900/50 text-${getStatusColor(invoice.status)}-400`}
                            >
                              {invoice.status}
                            </span>
                          </td>
          ...
          <div className="overflow-hidden rounded-lg border border-gray-800 bg-slate-900">
            <div className="flex items-center justify-between border-b border-gray-800 p-4">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Payment transactions in selected period
                </p>
              </div>
              <Link
                ...
                <tbody>
                  {financialData.transactions.length === 0 ? (
                    ...
                    financialData.transactions.slice(0, 15).map((transaction, index) => {
                      const invoice = financialData.invoices.find(
                        (inv) => inv._id === transaction.invoiceId
                      );
                      const customer = invoice
                        ? customerData[invoice._id] || { fullName: 'Customer' }
                        : { fullName: 'Customer' };
                      ...
                      return (
                        <tr key={index} className="border-t border-gray-800 hover:bg-slate-800/50">
                          ...
                          <td className="p-3">
                            <span
                              className={`rounded-full px-2 py-1 text-xs bg-${getTransactionStatusColor(transaction.transactionStatus)}-900/50 text-${getTransactionStatusColor(transaction.transactionStatus)}-400`}
                            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {financialData.summary.bankTransferPending} pending
                    </p>
                  </div>
                </div>
              </div>
