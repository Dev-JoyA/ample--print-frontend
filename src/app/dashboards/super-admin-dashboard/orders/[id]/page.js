          ...
          <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-3">
            <div className="rounded-lg border border-gray-800 bg-slate-900 p-4">
              ...
              <p className="mt-2 text-sm text-gray-400">
                ID: {order.userId?._id?.slice(-8) || 'N/A'}
              </p>
            </div>
            ...
            <div className="rounded-lg border border-gray-800 bg-slate-900 p-4">
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`inline-block rounded-full bg-${getStatusColor(order.status)}-900/50 px-3 py-1 text-xs font-medium text-${getStatusColor(order.status)}-400`}
                >
                  {order.status}
                </span>
              </div>
            ...
            <div className="rounded-lg border border-gray-800 bg-slate-900 p-4">
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`inline-block rounded-full bg-${getStatusColor(order.paymentStatus)}-900/50 px-3 py-1 text-xs font-medium text-${getStatusColor(order.paymentStatus)}-400`}
                >
          ...
          <div className="overflow-hidden rounded-lg border border-gray-800 bg-slate-900">
            <div className="border-b border-gray-800 px-6 py-4">
              <h2 className="text-lg font-semibold text-white">
                Order Items & Customization Briefs
              </h2>
            </div>
                ...
                return (
                  <div key={index} className="p-6">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      ...
                      {item.hasAdminResponse && (
                        <Link
                          href={`/briefs/responses?orderId=${orderId}&productId=${item.productId}`}
                        >
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-yellow-400">
                  {formatCurrency(order.remainingBalance || order.totalAmount)}
                </span>
              </div>
